...
int AlveoVnxLink::basicRecvSend(const std::string &remote_ip, uint16_t remote_udp, 
    ...
    uint32_t header;
    char *pkt_buffer_tx = new char[MAX_UDP_BUFFER_SIZE + 4];
    size_t size_left = size;
    size_t size_to_transfer = 0;
    size_t total_transferred_size = 0;

    char *pkt_buffer_rx = new char[MAX_UDP_BUFFER_SIZE + 4];
    size_t rx_size = 0;

    while (true) {
        // need to launch rx first
        this->rx->launchKernel(MAX_UDP_BUFFER_SIZE + 4);

        if (size_left > 0) { // for tx transfer data
            header = (size_left <= MAX_UDP_BUFFER_SIZE) << 31;
            if (size_left <= MAX_UDP_BUFFER_SIZE) {
                size_to_transfer = size_left;
                size_left -= size_left;
            ...
            // payload size
            header = header | size_to_transfer;
            std::cout << "tx header " << std::hex << header << std::dec << std::endl;
            *(uint32_t *) pkt_buffer_tx = header;
            memcpy(pkt_buffer_tx + 4, tx_buffer + total_transferred_size, size_to_transfer);
            total_transferred_size += size_to_transfer;
            this->tx->transferDataToKrnl(pkt_buffer_tx, size_to_transfer + 4);
            std::cout << "tx data transfered " << size_to_transfer << " bytes" << std::endl;
            std::cout << "tx size left: " << size_left << " total_transferred_size " << total_transferred_size << std::endl;
            this->tx->sendPacket(0);
            std::cout << "tx packet sent" << std::endl;
            usleep(1000);
        }

        this->rx->syncKernel(); // wait rx receive data // sync tx and rx

        this->rx->transferDataToHost(pkt_buffer_rx);

        uint32_t header = *(uint32_t *) pkt_buffer_rx;
        ...
        memcpy(rx_buffer + rx_size, pkt_buffer_rx + 4, pkt_size);
    ...
    delete[] pkt_buffer_tx;
    delete[] pkt_buffer_rx;
