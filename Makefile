...
LOOPDIR     = User_kernels/Loop_kernel/
...
ifeq (benchmark,$(DESIGN))
	...
	LIST_XO += $(LOOPDIR)$(TEMP_DIR)/krnl_loop.xo
endif
...
$(LOOPDIR)$(TEMP_DIR)/%.xo: $(LOOPDIR)src/*
	make -C $(LOOPDIR) all DEVICE=$(DEVICE) -j3
...
check-design:
	@if [[ ($(DESIGN) != "benchmark") && ($(DESIGN) != "basic") && ($(DESIGN) != "loop") ]]; then\
